...
const name = `get-validatorMisdemeanor-from-qa-${interval}`;

const ContractAddress = SYSTEM_CONTRACT_ADDRESS.ValidatorContract
const abi = ['event validatorMisdemeanor(address indexed validator, uint256 amount)'];
const iface = new ethers.utils.Interface(abi);

const eventName = 'validatorMisdemeanor(address,uint256)'
